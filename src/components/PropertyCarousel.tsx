  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 py-16 sm:py-20 lg:py-32">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-12 mb-10 sm:mb-16 lg:mb-20">
          <p className="text-[10px] sm:text-[11px] uppercase tracking-[0.3em] sm:tracking-[0.35em] text-white/90 mb-3 sm:mb-4 drop-shadow-sm">
            Our Properties
          </p>
          <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-6">
            <h2 className="font-display text-2xl sm:text-3xl md:text-4xl lg:text-5xl xl:text-6xl font-light text-white leading-tight">
              The Definition of{" "}
              <em className="italic font-normal text-brand-gold drop-shadow-[0_2px_8px_rgba(201,169,98,0.3)]">Luxury</em>
              <br className="hidden sm:block" />
              <span className="sm:hidden"> </span>
              Modern Living
            </h2>

            {/* Navigation Arrows - Four Seasons style with gold hover - desktop only */}
            <div className="hidden lg:flex items-center gap-3 flex-shrink-0">
              <button
                onClick={prevSlide}
                className="w-12 h-12 border border-white/50 flex items-center justify-center hover:bg-brand-gold hover:border-brand-gold group transition-all duration-300"
                aria-label="Previous"
              >
        ...
        {/* Progress Indicator - Four Seasons style with gold accent */}
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-12 mt-8 sm:mt-12 lg:mt-16">
          <div className="flex items-center gap-3 sm:gap-4">
            <span className="text-[10px] sm:text-[11px] text-brand-gold tracking-wider font-medium">
              {String(activeIndex + 1).padStart(2, "0")}
            </span>
            <div className="flex-1 h-[2px] bg-white/50 relative overflow-hidden">
              <div
                className="absolute top-0 left-0 h-full bg-gradient-to-r from-brand-gold via-brand-goldLight to-brand-gold transition-all duration-500"
                style={{ width: `${((activeIndex + 1) / destinations.length) * 100}%` }}
              />
            </div>
            <span className="text-[10px] sm:text-[11px] text-white/90 tracking-wider">
        ...
        <div className="flex lg:hidden justify-center gap-4 mt-6 sm:mt-8 px-4 sm:px-6">
          <button
            onClick={prevSlide}
            className="w-12 h-12 sm:w-14 sm:h-14 border border-white/50 flex items-center justify-center active:bg-brand-gold/30 transition-colors"
            aria-label="Previous"
          >
...
function PropertyCard({ property, isActive, onClick, isMobile }: PropertyCardProps) {
  return (
    <div
      className={cn(
    ...
    >
      ...
      <div className="text-white">
        <p className="text-[9px] sm:text-[10px] uppercase tracking-[0.2em] sm:tracking-[0.25em] text-white/90 mb-1.5 sm:mb-2 drop-shadow-sm">
          {property.subtitle}
        </p>
        <h3 className="font-display text-lg sm:text-xl lg:text-2xl font-light mb-2 sm:mb-3 drop-shadow-md">
          {property.shortTitle}
        </h3>
        <p className="text-xs sm:text-sm text-white/90 leading-relaxed line-clamp-2 mb-3 sm:mb-4">
          {property.copy}
        </p>
        <Link
          href={`/destinations/${property.slug}`}
          className="inline-flex items-center gap-2 text-[10px] sm:text-[11px] uppercase tracking-[0.15em] text-white/90 hover:text-white transition-colors group/link"
        >
          <span>Explore</span>
